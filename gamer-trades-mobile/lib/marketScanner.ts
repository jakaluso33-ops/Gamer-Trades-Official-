...
/**
 * Runs the same strategy detectors used on a single chart across every symbol in the given
 * list simultaneously, ticking each symbol's simulated candle series forward on an interval.
 * Powers the cross-market "master trader" scanner feed — a live view of every setup forming
 * across all 6 asset classes at once, not just the one instrument a user happens to have open.
 */
  ...
  useEffect(() => {
    const tick = () => {
      ...
      for (const s of symbols) {
        ...
        if (signals[0]) out.push({ symbol: s.symbol, name: s.name, assetClass: s.class, signal: signals[0] });
      }
      setResults(out);
    };
    tick();
