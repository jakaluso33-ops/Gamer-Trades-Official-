...
import CandlePatternDiagram from '../../components/CandlePatternDiagram';
...
export default function AcademyScreen() {
  ...
  const locked = !!selected.provenProfitable && !isPro;
      ...
      {mode === 'strategies' && (
        <>
            ...
            {STRATEGIES.map(s => (
              ...
              >
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                  ...
                  {s.provenProfitable && <BodyText size={11}>🔒</BodyText>}
                </View>
                {(s.popular || s.provenProfitable) && (
                  <BodyText color={s.provenProfitable ? colors.gold : colors.green} size={9} weight="semibold" style={{ marginTop: 2 }}>
                    {s.provenProfitable ? '★ PROVEN PROFITABLE' : '🔥 POPULAR'}
                  </BodyText>
                )}
              </Pressable>
            ))}
          ...
          <Card borderColor={selected.color}>
            ...
            {locked ? (
              <View style={{ padding: 16, backgroundColor: `${colors.gold}0a`, borderWidth: 2, borderColor: `${colors.gold}55`, alignItems: 'center' }}>
                <PixelText size={22} style={{ marginBottom: 8 }}>🔒</PixelText>
                <PixelText color={colors.gold} size={10} glow style={{ textAlign: 'center', marginBottom: 8 }}>PROVEN-PROFITABLE STRATEGY</PixelText>
                <BodyText color={colors.text} size={12.5} style={{ textAlign: 'center', marginBottom: 14 }}>
                  This is one of the small set of strategies with a genuinely rigorous, long-documented track record — not just popular, but proven. Full rules, entry/exit logic, and live detection are Pro/Legend only.
                </BodyText>
                <PixelButton color={colors.gold} onPress={handleUpgrade} disabled={upgradeBusy} style={{ alignSelf: 'stretch' }}>
      ...
      {mode === 'candles' && (
        <>
          ...
          <Card borderColor={selectedPattern.color}>
            ...
            <View style={{ alignItems: 'center', paddingVertical: 14, marginBottom: 14, backgroundColor: colors.bg, borderWidth: 1, borderColor: colors.border }}>
              <CandlePatternDiagram id={selectedPattern.id} height={170} />
              <BodyText color={colors.muted} size={10} style={{ marginTop: 8 }}>WHAT THE PATTERN LOOKS LIKE</BodyText>
            </View>
