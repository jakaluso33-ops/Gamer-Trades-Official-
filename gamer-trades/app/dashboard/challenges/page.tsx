  ...
  return (
    <div className="grid-bg" style={{ minHeight: '100%' }}>
      <div style={{ marginBottom: '20px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
        <div>
          <div style={{ fontSize: '10px', color: '#64748b', marginBottom: '6px' }}>◆ TAILORED TO YOU</div>
          <h1 className="font-pixel" style={{ fontSize: '12px', color: '#00ff88', textShadow: '0 0 12px #00ff88', margin: 0 }}>GOALS &amp; TASKS</h1>
        </div>
        {syncing && <div style={{ fontSize: '10px', color: '#64748b' }}>syncing...</div>}
      </div>

      {/* Goals */}
      <div className="retro-card" style={{ padding: '16px', marginBottom: '20px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
          <span style={{ fontSize: '11px', color: '#8b5cf6', textShadow: '0 0 8px #8b5cf6' }}>★ YOUR GOALS</span>
          {goals.length < 3 && availableTemplates.length > 0 && (
            <button onClick={() => setShowPicker(v => !v)} className="pixel-btn pixel-btn-blue" style={{ fontSize: '10px', padding: '6px 10px' }}>
          ...
          {goals.map(g => {
            ...
            return (
              <div key={g.id} style={{ padding: '12px', background: `${t.color}0a`, border: `2px solid ${t.color}`, boxShadow: `3px 3px 0 #000, 0 0 8px ${t.color}33` }}>
                ...
                <div style={{ fontSize: '11px', color: t.color, textShadow: `0 0 6px ${t.color}`, marginTop: '6px' }}>{t.label}</div>
                <div style={{ fontSize: '9px', color: '#64748b', marginTop: '6px', lineHeight: 1.6 }}>{t.description}</div>
              </div>
            );
          })}
        </div>
        ...
        {showPicker && (
          <div style={{ marginTop: '14px', paddingTop: '14px', borderTop: '1px solid #1e3a5f', display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: '10px' }}>
            {availableTemplates.map(t => (
              ...
              >
                <span style={{ fontSize: '18px' }}>{t.icon}</span>
                <div style={{ fontSize: '11px', color: t.color, marginTop: '6px' }}>{t.label}</div>
                <div style={{ fontSize: '9px', color: '#64748b', marginTop: '6px', lineHeight: 1.6 }}>{t.description}</div>
              </div>
            ))}
          </div>
      ...
      {visibleTasks.length === 0 ? (
        <div className="retro-card" style={{ padding: '32px', textAlign: 'center', fontSize: '11px', color: '#64748b' }}>
            ...
            return (
              ...
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}>
                  <span style={{ fontSize: '11px', color: done ? accent : '#e2e8f0', textShadow: done ? `0 0 6px ${accent}` : 'none' }}>
                    {done ? '✓ ' : ''}{task.title}
                  </span>
                  <span style={{ fontSize: '9px', color: '#ffd700', textShadow: '0 0 6px #ffd700' }}>+{task.xp_reward} XP</span>
                </div>
                <div style={{ fontSize: '9px', color: '#64748b', marginBottom: '8px' }}>{task.description}</div>
...
function PixelLoading() {
  return <div style={{ fontSize: '12px', color: '#00ffff', textShadow: '0 0 8px #00ffff' }}>LOADING...</div>;
}
