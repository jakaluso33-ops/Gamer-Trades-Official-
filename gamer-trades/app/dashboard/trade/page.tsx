  ...
  return (
    <div className="grid-bg" style={{ minHeight: '100%' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
        <div>
          <div style={{ fontSize: '10px', color: '#64748b', marginBottom: '4px' }}>◈ TRADING ARENA</div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            <span style={{ fontSize: '12px', color: '#00ffff', textShadow: '0 0 10px #00ffff' }}>
              {selected.symbol}
            </span>
            <span
              style={{
        ...
        <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
          <span style={{ fontSize: '11px', color: '#64748b' }}>CASH:</span>
          <span style={{ fontSize: '9px', color: '#e2e8f0' }}>
            ${(portfolio?.cash_balance ?? 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}
          </span>
          <span style={{ fontSize: '11px', color: '#64748b', marginLeft: '8px' }}>OPEN P&L:</span>
          <span
            style={{
              fontSize: '9px',
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 260px', gap: '12px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          ...
          <div className="retro-card" style={{ overflow: 'hidden' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '8px 12px', borderBottom: '1px solid #1e3a5f', flexWrap: 'wrap' }}>
              <span style={{ fontSize: '10px', color: '#8b5cf6', textShadow: '0 0 6px #8b5cf6' }}>🧠 SCANNER:</span>
            ...
            {latestSignal && (() => {
              ...
              return (
                <div style={{ padding: '10px 12px', borderTop: '1px solid #1e3a5f', background: `${color}0a`, display: 'flex', alignItems: 'center', gap: '10px' }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: '11px', color, textShadow: `0 0 6px ${color}` }}>
                      {strat?.name ?? latestSignal.strategyId} — {latestSignal.label}
                    </div>
                    <div style={{ fontSize: '9px', color: '#64748b', marginTop: '3px' }}>{latestSignal.detail}</div>
                  </div>
                  <Link href="/dashboard/academy" style={{ fontSize: '9px', color, textDecoration: 'none' }}>▶ LEARN THIS</Link>
                </div>
              );
            })()}
            ...
            {activeIndicators.includes('RSI') && (
              <div style={{ borderTop: '1px solid #1e3a5f', padding: '6px 12px' }}>
                <span style={{ fontSize: '10px', color: '#8b5cf6' }}>RSI(14): </span>
                <span style={{ fontSize: '11px', color: '#e2e8f0' }}>54.2</span>
                <div style={{ marginTop: '4px', height: '30px', background: '#0a0e1a', border: '1px solid #1e3a5f', position: 'relative', overflow: 'hidden' }}>
                  ...
                  {['30', '50', '70'].map((v, i) => (
                    <span key={v} style={{ position: 'absolute', top: '2px', left: `${[30, 50, 70][i]}%`, fontSize: '9px', color: '#64748b', transform: 'translateX(-50%)' }}>{v}</span>
                  ))}
                </div>
              </div>
            )}
          ...
          <div className="retro-card" style={{ overflow: 'hidden' }}>
            <div style={{ padding: '8px 12px', borderBottom: '2px solid #1e3a5f', display: 'flex', justifyContent: 'space-between' }}>
              <span style={{ fontSize: '11px', color: '#00aaff', textShadow: '0 0 8px #00aaff' }}>◈ POSITIONS</span>
              <span style={{ fontSize: '10px', color: totalPnL >= 0 ? '#00ff88' : '#ff3355' }}>
                P&L: {totalPnL >= 0 ? '+' : ''}${totalPnL.toFixed(2)}
              </span>
            </div>

            {openTrades.length === 0 ? (
              <div style={{ padding: '20px', textAlign: 'center', fontSize: '11px', color: '#1e3a5f' }}>
                NO OPEN POSITIONS
                <br /><br />
                <span style={{ fontSize: '10px' }}>Place an order to get started ▶</span>
              </div>
            ) : (
              <>
                <div style={{ display: 'grid', gridTemplateColumns: '70px 45px 50px 80px 80px 70px 60px', gap: '4px', padding: '5px 10px', borderBottom: '1px solid #1e3a5f', fontSize: '9px', color: '#64748b' }}>
                ...
                {openTrades.map(t => {
                  ...
                  const up = pnl >= 0;
                  return (
                    <div key={t.id} style={{ display: 'grid', gridTemplateColumns: '70px 45px 50px 80px 80px 70px 60px', gap: '4px', padding: '7px 10px', borderBottom: '1px solid #0f1629', alignItems: 'center', fontSize: '11px' }}>
                      <span style={{ color: '#00aaff' }}>{t.symbol}</span>
                      <span style={{ color: t.direction === 'long' ? '#00ff88' : '#ff3355', fontSize: '10px' }}>{t.direction === 'long' ? 'BUY' : 'SELL'}</span>
          ...
          <div className="retro-card" style={{ padding: '10px 12px' }}>
            <div style={{ fontSize: '10px', color: '#64748b', marginBottom: '6px' }}>◎ ORDER LOG</div>
            {orderLog.length === 0
              ? <div style={{ fontSize: '10px', color: '#1e3a5f' }}>No orders yet...</div>
              : orderLog.map((log, i) => (
                  <div key={i} style={{ fontSize: '10px', color: i === 0 ? '#00ff88' : '#64748b', padding: '2px 0', borderBottom: '1px solid #0f1629' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          <div className="retro-card" style={{ padding: '14px' }}>
            <div style={{ fontSize: '11px', color: '#00aaff', textShadow: '0 0 8px #00aaff', marginBottom: '12px' }}>
              ◈ ORDER PANEL
            </div>
            {orderError && (
              <div style={{ fontSize: '10px', color: '#ff3355', padding: '8px', marginBottom: '10px', background: '#ff335511', border: '1px solid #ff335544' }}>
                ⚠ {orderError}
              </div>
            )}
            <OrderPanel symbol={selected.symbol} currentPrice={livePrice} onOrder={handleOrder} />
            {placingOrder && <div style={{ fontSize: '9px', color: '#64748b', marginTop: '6px' }}>Placing order...</div>}
          </div>

          {/* Symbol info */}
          <div className="retro-card" style={{ padding: '12px' }}>
            <div style={{ fontSize: '10px', color: '#64748b', marginBottom: '8px' }}>◎ MARKET INFO</div>
            {[
          ...
          <div className="retro-card" style={{ padding: '12px', borderColor: '#ffd70044', boxShadow: '4px 4px 0 #000, 0 0 8px #ffd70022' }}>
            <div style={{ fontSize: '10px', color: '#ffd700', textShadow: '0 0 6px #ffd700', marginBottom: '6px' }}>
              💡 PRO TIP
            </div>
