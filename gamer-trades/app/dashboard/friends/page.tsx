  ...
  return (
    <div className="grid-bg" style={{ minHeight: '100%' }}>
      <div style={{ marginBottom: '20px' }}>
        <div style={{ fontSize: '6px', color: '#64748b', marginBottom: '6px' }}>♥ SOCIAL</div>
        <h1 style={{ fontSize: '14px', color: '#00ff88', textShadow: '0 0 12px #00ff88', margin: 0 }}>FRIENDS</h1>
      </div>

      {toast && (
        <div style={{ position: 'fixed', top: '60px', right: '20px', zIndex: 200, padding: '10px 16px', background: '#0f1629', border: '2px solid #00ff88', boxShadow: '4px 4px 0 #000, 0 0 12px #00ff8844', fontSize: '7px', color: '#00ff88' }}>
          {toast}
        </div>
      )}
        ...
        <div className="retro-card" style={{ padding: '16px', marginBottom: '16px' }}>
          <input
            ...
            style={{
              ...
              fontSize: '8px', padding: '10px', fontFamily: 'inherit', outline: 'none', boxSizing: 'border-box',
            }}
          />
        </div>
      )}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          ...
          friends.length === 0
            ...
            : friends.map(f => (
              <PersonRow key={f.id} profile={f.otherProfile} busy={busyId === f.id}>
                <button onClick={() => challengeFriend(f.otherProfile.id)} className="pixel-btn pixel-btn-green" style={{ fontSize: '6px', padding: '7px 10px' }}>⚔ CHALLENGE</button>
                <button onClick={() => removeFriend(f.id)} className="pixel-btn pixel-btn-red" style={{ fontSize: '6px', padding: '7px 10px' }}>✕ REMOVE</button>
              </PersonRow>
            ))
        )}
        ...
        {tab === 'incoming' && (
          incoming.length === 0
            ...
            : incoming.map(f => (
              <PersonRow key={f.id} profile={f.otherProfile} busy={busyId === f.id}>
                <button onClick={() => respond(f.id, 'accepted')} className="pixel-btn pixel-btn-green" style={{ fontSize: '6px', padding: '7px 10px' }}>✓ ACCEPT</button>
                <button onClick={() => respond(f.id, 'declined')} className="pixel-btn pixel-btn-red" style={{ fontSize: '6px', padding: '7px 10px' }}>✕ DECLINE</button>
              </PersonRow>
            ))
        )}
        ...
        {tab === 'outgoing' && (
          outgoing.length === 0
            ...
            : outgoing.map(f => (
              <PersonRow key={f.id} profile={f.otherProfile} busy={busyId === f.id}>
                <span style={{ fontSize: '6px', color: '#ffd700' }}>PENDING...</span>
                <button onClick={() => removeFriend(f.id)} className="pixel-btn pixel-btn-red" style={{ fontSize: '6px', padding: '7px 10px' }}>✕ CANCEL</button>
              </PersonRow>
            ))
        )}
            ...
            : results.length === 0
              ...
              : results.map(p => (
                <PersonRow key={p.id} profile={p} busy={busyId === p.id}>
                  {knownIds.has(p.id)
                    ? <span style={{ fontSize: '6px', color: '#64748b' }}>ALREADY CONNECTED</span>
                    : <button onClick={() => sendRequest(p.id)} className="pixel-btn pixel-btn-blue" style={{ fontSize: '6px', padding: '7px 10px' }}>+ ADD FRIEND</button>}
                </PersonRow>
              ))
        )}
      </div>
...
function PersonRow({ profile, busy, children }: { profile: Profile; busy: boolean; children: React.ReactNode }) {
  return (
    <div className="retro-card" style={{ padding: '12px', display: 'flex', alignItems: 'center', gap: '14px', opacity: busy ? 0.5 : 1 }}>
      ...
      <div style={{ flex: 1 }}>
        <div style={{ fontSize: '8px', color: '#00ffff', textShadow: '0 0 6px #00ffff' }}>{profile.username}</div>
        <div style={{ fontSize: '5px', color: '#64748b', marginTop: '2px' }}>
...
function EmptyState({ text }: { text: string }) {
  return (
    <div className="retro-card" style={{ padding: '24px', textAlign: 'center', fontSize: '7px', color: '#64748b' }}>
      {text}
    </div>
  );
}
