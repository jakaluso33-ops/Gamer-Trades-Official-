import Link from 'next/link';
import { TERMS_OF_SERVICE, LEGAL_UPDATED } from '@/lib/legalContent';

export const metadata = { title: 'Terms of Service — GamerTrades' };

export default function TermsPage() {
  return (
    <main style={{ minHeight: '100vh', background: '#0a0e1a', padding: '32px 20px' }}>
      <div style={{ maxWidth: '640px', margin: '0 auto' }}>
        <Link href="/" style={{ fontSize: '6px', color: '#64748b', textDecoration: 'none' }}>◀ BACK TO HOME</Link>
        <h1 style={{ fontSize: '16px', color: '#00ffff', textShadow: '0 0 10px #00ffff', marginTop: '20px', marginBottom: '6px' }}>
          TERMS OF SERVICE
        </h1>
        <p style={{ fontSize: '6px', color: '#64748b', marginBottom: '24px' }}>Last updated: {LEGAL_UPDATED}</p>
        {TERMS_OF_SERVICE.map(section => (
          <section key={section.heading} style={{ marginBottom: '20px' }}>
            <h2 style={{ fontSize: '9px', color: '#00aaff', textShadow: '0 0 8px #00aaff', marginBottom: '8px' }}>
              {section.heading}
            </h2>
            <p style={{ fontSize: '7px', color: '#e2e8f0', lineHeight: 1.9 }}>{section.body}</p>
          </section>
        ))}
      </div>
    </main>
  );
}
