          ...
          <div className="retro-card" style={{
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '24px' }}>
              <div style={{ padding: '12px', background: '#0a0e1a', border: `2px solid ${myPnl >= 0 ? '#00ff8844' : '#ff335544'}` }}>
                <div style={{ fontSize: '9px', color: '#64748b', marginBottom: '4px' }}>YOUR P&L</div>
                <div style={{ fontSize: '10px', color: myPnl >= 0 ? '#00ff88' : '#ff3355' }}>{myPnl >= 0 ? '+' : ''}${myPnl.toFixed(2)}</div>
              </div>
              <div style={{ padding: '12px', background: '#0a0e1a', border: `2px solid ${oppPnl >= 0 ? '#00ff8844' : '#ff335544'}` }}>
                <div style={{ fontSize: '9px', color: '#64748b', marginBottom: '4px' }}>{opponent?.username ?? 'OPPONENT'} P&L</div>
                <div style={{ fontSize: '10px', color: oppPnl >= 0 ? '#00ff88' : '#ff3355' }}>{oppPnl >= 0 ? '+' : ''}${oppPnl.toFixed(2)}</div>
              </div>
            </div>
            {iWon && <div style={{ padding: '8px', background: '#8b5cf622', border: '2px solid #8b5cf6', marginBottom: '16px', fontSize: '11px', color: '#8b5cf6' }}>+150 XP EARNED!</div>}
            <button onClick={exitToLobby} className="pixel-btn pixel-btn-blue" style={{ width: '100%', fontSize: '11px', padding: '10px' }}>◀ BACK TO FRIENDS LOBBY</button>
          </div>
        </div>
      );
    }
    ...
    return (
      <div className="grid-bg" style={{ minHeight: '100%' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
          <div style={{ fontSize: '12px', color: '#ff3355', textShadow: '0 0 10px #ff3355' }}>⚔ LIVE PVP BATTLE</div>
          <div style={{ textAlign: 'center' }}>
            <div style={{ fontSize: '10px', color: '#64748b', marginBottom: '2px' }}>TIME REMAINING</div>
            <div style={{ fontSize: '12px', color: timeLeft < 30 ? '#ff3355' : '#ffd700', textShadow: `0 0 12px ${timeLeft < 30 ? '#ff3355' : '#ffd700'}`, fontVariantNumeric: 'tabular-nums' }}>
              {formatTime(timeLeft)}
            </div>
          </div>
          <div style={{ fontSize: '10px', color: '#00ffff' }}>{MODES.find(m => m.id === activeMatch.mode)?.label}</div>
        </div>
        ...
        <div className="retro-card" style={{ padding: '20px', marginBottom: '16px', borderColor: '#8b5cf6', boxShadow: '4px 4px 0 #000, 0 0 16px #8b5cf633' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr auto 1fr', gap: '20px', alignItems: 'center' }}>
            <div style={{ textAlign: 'center' }}>
              ...
              <div style={{ fontSize: '12px', color: '#00ffff', textShadow: '0 0 8px #00ffff', marginBottom: '4px' }}>{profile?.username ?? 'YOU'}</div>
            ...
            <div style={{ textAlign: 'center' }}>
              <div style={{ fontSize: '28px', marginBottom: '6px' }}>🕹</div>
              <div style={{ fontSize: '12px', color: '#ffd700', textShadow: '0 0 8px #ffd700', marginBottom: '4px' }}>{opponent?.username ?? '...'}</div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 280px', gap: '12px' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            ...
            <div className="retro-card" style={{ padding: '12px' }}>
              ...
              {trades.length === 0
                ...
                : trades.map((t, i) => (
                  <div key={t.id} style={{ display: 'flex', gap: '8px', padding: '4px 0', borderBottom: '1px solid #0f1629', fontSize: '10px', opacity: 1 - i * 0.04 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            <div className="retro-card" style={{ padding: '14px' }}>
              <div style={{ fontSize: '11px', color: '#00ffff', textShadow: '0 0 8px #00ffff', marginBottom: '12px' }}>YOUR MOVES</div>
    ...
    <div className="grid-bg" style={{ minHeight: '100%' }}>
      {incomingInvites.length > 0 && (
        <div className="retro-card" style={{ padding: '16px', marginBottom: '16px', borderColor: '#ff3355', boxShadow: '4px 4px 0 #000, 0 0 12px #ff335544' }}>
          <div style={{ fontSize: '11px', color: '#ff3355', textShadow: '0 0 8px #ff3355', marginBottom: '10px' }}>⚔ INCOMING CHALLENGES</div>
          {incomingInvites.map(inv => (
            <div key={inv.id} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '8px 0', borderBottom: '1px solid #0f1629' }}>
              ...
              <button onClick={() => acceptInvite(inv)} className="pixel-btn pixel-btn-green" style={{ fontSize: '10px', padding: '7px 10px' }}>✓ ACCEPT</button>
              <button onClick={() => declineInvite(inv)} className="pixel-btn pixel-btn-red" style={{ fontSize: '10px', padding: '7px 10px' }}>✕ DECLINE</button>
            </div>
          ))}
        </div>
      )}
      ...
      {outgoingPending.length > 0 && (
        <div className="retro-card" style={{ padding: '16px', marginBottom: '16px', borderColor: '#ffd700', boxShadow: '4px 4px 0 #000, 0 0 12px #ffd70044' }}>
          <div style={{ fontSize: '11px', color: '#ffd700', textShadow: '0 0 8px #ffd700', marginBottom: '10px' }}>⏳ WAITING FOR RESPONSE</div>
          {outgoingPending.map(inv => (
            <div key={inv.id} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '8px 0', borderBottom: '1px solid #0f1629' }}>
              <span style={{ fontSize: '11px', color: '#e2e8f0', flex: 1 }}>
                Challenged <b style={{ color: '#00ffff' }}>{inv.opponent.username}</b> — {MODES.find(m => m.id === inv.mode)?.label}
              </span>
              <button onClick={() => cancelMyPendingMatch(inv.id)} className="pixel-btn pixel-btn-red" style={{ fontSize: '10px', padding: '7px 10px' }}>✕ CANCEL</button>
            </div>
          ))}
        </div>
      )}
      ...
      {friends.length === 0 ? (
        <div className="retro-card" style={{ padding: '32px', textAlign: 'center' }}>
          <div style={{ fontSize: '11px', color: '#64748b', marginBottom: '10px' }}>You have no friends added yet.</div>
          <a href="/dashboard/friends" style={{ fontSize: '11px', color: '#00ffff', textShadow: '0 0 6px #00ffff' }}>▶ GO ADD FRIENDS</a>
        </div>
      ) : (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            <div style={{ fontSize: '11px', color: '#64748b', marginBottom: '4px' }}>CHOOSE A FRIEND</div>
            {friends.map(f => (
              <div
                key={f.id}
              ...
              >
                ...
                <div>
                  <div style={{ fontSize: '12px', color: '#00ffff' }}>{f.username}</div>
                  <div style={{ fontSize: '9px', color: '#64748b' }}>LVL {f.level} · {f.total_wins}W-{f.total_losses}L</div>
                </div>
              </div>
            ))}
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
              {MODES.map(mode => (
                ...
                >
                  <div style={{ fontSize: '11px', color: selectedMode.id === mode.id ? '#00ffff' : '#e2e8f0', marginBottom: '4px' }}>{mode.label}</div>
                  <div style={{ fontSize: '12px', color: '#ffd700' }}>{mode.duration}</div>
                </div>
              ))}
            </div>
            ...
            <div className="retro-card" style={{ padding: '16px', flex: 1 }}>
              {selectedFriend ? (
                <>
                  <div style={{ fontSize: '11px', color: '#ffd700', marginBottom: '12px', textAlign: 'center' }}>⚔ CHALLENGE {selectedFriend.username}</div>
                ...
                <div style={{ fontSize: '10px', color: '#64748b', textAlign: 'center' }}>Select a friend to challenge.</div>
              )}
            </div>
          </div>
        </div>
