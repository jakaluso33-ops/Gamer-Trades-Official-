...
export default function InsightsCard() {
  ...
  return (
    ...
    >
      <div style={{ fontSize: '7px', color: '#8b5cf6', textShadow: '0 0 8px #8b5cf6', marginBottom: '12px' }}>
        🧠 COACH INSIGHTS
      </div>

      {insights === null && (
        <div style={{ fontSize: '6px', color: '#64748b' }}>Analyzing your recent activity...</div>
      )}

      {insights !== null && insights.length === 0 && (
        <div style={{ fontSize: '6px', color: '#64748b' }}>Nothing urgent — keep trading and check back soon.</div>
      )}
            ...
            return (
              ...
              >
                <div style={{ display: 'flex', gap: '8px', alignItems: 'flex-start', marginBottom: '6px' }}>
                  ...
                  <span style={{ fontSize: '7px', color, textShadow: `0 0 6px ${color}` }}>{insight.title}</span>
                </div>
                <div style={{ fontSize: '5px', color: '#64748b', lineHeight: 1.7, marginBottom: '8px' }}>
                  {insight.message}
                </div>
                <Link
                  href={ACTION_HREF[insight.actionKey]}
                  style={{ fontSize: '5px', color, textDecoration: 'none', textShadow: `0 0 6px ${color}` }}
                >
                  ▶ {insight.actionLabel}
                </Link>
              </div>
